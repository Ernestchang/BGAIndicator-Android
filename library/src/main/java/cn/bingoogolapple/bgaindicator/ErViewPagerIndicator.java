...
public class ErViewPagerIndicator extends LinearLayout implements View.OnClickListener, View.OnFocusChangeListener {
    private static final String TAG = ErViewPagerIndicator.class.getSimpleName();
    private final int EEINID = 0xffff00;
    ...
    public ErViewPagerIndicator(Context context) {
        this(context, null);
    }

    public ErViewPagerIndicator(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void initCustomAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.ErViewPagerIndicator);
    ...
    public void initCustomAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.ErViewPagerIndicator_er_triangleColor) {
            mTriangleColor = typedArray.getColor(attr, mTriangleColor);
        } else if (attr == R.styleable.ErViewPagerIndicator_er_triangleHorizontalMargin) {
            /**
             * getDimension和getDimensionPixelOffset的功能差不多,都是获取某个dimen的值,如果是dp或sp的单位,
             * 将其乘以density,如果是px,则不乘;两个函数的区别是一个返回float,一个返回int.
        ...
        } else if (attr == R.styleable.ErViewPagerIndicator_er_triangleHeight) {
            mTriangleHeight = typedArray.getDimensionPixelOffset(attr, mTriangleHeight);
        } else if (attr == R.styleable.ErViewPagerIndicator_er_textColor) {
            mTextColor = typedArray.getColorStateList(attr);
        } else if (attr == R.styleable.ErViewPagerIndicator_er_textSizeNormal) {
            mTextSizeNormal = typedArray.getDimensionPixelOffset(attr, mTextSizeNormal);
        } else if (attr == R.styleable.ErViewPagerIndicator_er_textSizeSelected) {
            mTextSizeSelected = typedArray.getDimensionPixelOffset(attr, mTextSizeSelected);
        } else if (attr == R.styleable.ErViewPagerIndicator_er_hasDivider) {
            mHasDivider = typedArray.getBoolean(attr, mHasDivider);
        } else if (attr == R.styleable.ErViewPagerIndicator_er_dividerColor) {
            mDividerColor = typedArray.getColor(attr, mDividerColor);
        } else if (attr == R.styleable.ErViewPagerIndicator_er_dividerWidth) {
            mDividerWidth = typedArray.getDimensionPixelOffset(attr, mDividerWidth);
        } else if (attr == R.styleable.ErViewPagerIndicator_er_dividerVerticalMargin) {
            mDividerVerticalMargin = typedArray.getDimensionPixelOffset(attr, mDividerVerticalMargin);
        }
    }
    ...
    private void initTab(int currentTab) {
        for (int index = 0; index < mTabCount; index++) {
            ...
            // setId to a layoutview then you can use findViewById get this view
            tabIndicator.setId(EEINID + index);
            tabIndicator.setOnClickListener(this);

            TextView titleTv = (TextView) tabIndicator.findViewById(R.id.tv_indicator_title);
            tabIndicator.setTag(titleTv);
            if (mTextColor != null) {
    ...
    private void setCurrentTab(int index) {
        if (mCurrentTabIndex != index && index > -1 && index < mTabCount) {
            // through setId() to get the layoutView
            View oldTab = findViewById(EEINID + mCurrentTabIndex);
            resetTab(oldTab, false);

            mCurrentTabIndex = index;
            View newTab = findViewById(EEINID + mCurrentTabIndex);
...
//        TextView tv = (TextView) tab.findViewById(R.id.tv_indicator_title);
    ...
    public void onClick(View v) {
        int currentTabIndex = v.getId() - EEINID;
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        if (v == this && hasFocus) {
            findViewById(EEINID + mCurrentTabIndex).requestFocus();
            return;
        } else if (hasFocus) {
...
//        mPath.close();
        ...
        mPaintFooterTriangle.setStrokeWidth(mTriangleHeight);

        canvas.drawLine(left_x, top_y, right_x, top_y, mPaintFooterTriangle);
